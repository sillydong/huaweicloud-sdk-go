...
func (t *Template) Validate() error {
	...
	message := fmt.Sprintf("Template format version:%s not found.", invalid)
	return gophercloud.NewSystemCommonError("Com.2000", message)
}
...
func (t *Template) getFileContents(te interface{}, ignoreIf igFunc, recurse bool) error {
	...
	default:
		...
		message := fmt.Sprintf(gophercloud.CE_ErrUnexpectedTypeMessage, "", fmt.Sprintf("%v", reflect.TypeOf(te)))
