...
func (t *TE) Parse() error {
	...
	if jerr := json.Unmarshal(t.Bin, &t.Parsed); jerr != nil {
		if yerr := yaml.Unmarshal(t.Bin, &t.Parsed); yerr != nil {
			...
			return gophercloud.NewSystemCommonError("Com.2000", "Data in neither json nor yaml format.")
		}
	}
	return t.Validate()
}
...
func toStringKeys(m interface{}) (map[string]interface{}, error) {
	...
	default:
		...
		expected := "map[string]interface{}/map[interface{}]interface{}"
		actual := fmt.Sprintf("%v", reflect.TypeOf(m))
