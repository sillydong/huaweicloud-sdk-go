...
func (opts CreateOpts) ToLBMonitorCreateMap() (map[string]interface{}, error) {
	if opts.Type == TypeHTTP || opts.Type == TypeHTTPS {
		if opts.URLPath == "" {
			message := fmt.Sprintf(gophercloud.CE_MissingInputMessage, "monitors.CreateOpts.URLPath")
		...
		if opts.ExpectedCodes == "" {
			message := fmt.Sprintf(gophercloud.CE_MissingInputMessage, "monitors.CreateOpts.ExpectedCodes")
			err := gophercloud.NewSystemCommonError(gophercloud.CE_MissingInputCode, message)
			return nil, err
			//			err := gophercloud.ErrMissingInput{}
	...
	if opts.Delay < opts.Timeout {
		...
		message := fmt.Sprintf(gophercloud.CE_InvalidInputMessage, "monitors.CreateOpts.Delay/monitors.CreateOpts.Timeout Delay must be greater than or equal to timeout")
...
func (opts UpdateOpts) ToLBMonitorUpdateMap() (map[string]interface{}, error) {
	if opts.Delay > 0 && opts.Timeout > 0 && opts.Delay < opts.Timeout {
		...
		value := fmt.Sprintf("monitors.CreateOpts.Delay/monitors.CreateOpts.Timeout:%d/%d Delay must be greater than or equal to timeout", opts.Delay, opts.Timeout)
